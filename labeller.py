...
transactions_v1 = pd.read_csv('data/transactions.csv')
...
is_cancel_all = [[] for i in range(len(targets))]
            ...
            for z, target in enumerate(targets):
                ...
                for j in range(len(transaction_dates)-1,-1,-1):
                    ...
                    #如果到期日在目標月份，不是取消，同時交易日要在目標月以前
                        ...
                        is_cancel_all[z].append(is_cancels[j])
                        last_1_is_churn = -1
                        last_2_is_churn = -1
                        last_3_is_churn = -1
                        last_4_is_churn = -1
...
for i, target in enumerate(targets):
    ...
    out['autorenew_&_not_cancel'] = ((out.is_auto_renew == 1) == (out.is_cancel == 0)).astype(np.int8)
    out['notAutorenew_&_cancel'] = ((out.is_auto_renew == 0) == (out.is_cancel == 1)).astype(np.int8)
    out.to_csv('train_v5_{}.csv'.format(target), index=False)